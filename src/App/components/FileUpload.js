...
const FileUpload = (props) => {
    ...
    const uploadAvatar = e => {
        ...
        const uploadTask = storage().ref(`/${props.type}/${token}/user${props.type}_` + imageAsFile.name).put(imageAsFile)
        ...
        uploadTask.on('state_changed', (snapShot) => {
            ...
            storage().ref(`/${props.type}/${token}`).child(`user${props.type}_` + imageAsFile.name).getDownloadURL()
            .then(fireBaseUrl => {
                return fireBaseUrl;
            })
            .then(imageUrl => {
    ...
    return (
        <Form>
            ...
            <Button variant={"primary"} onClick={uploadAvatar} disabled={!imageAsFile}>Salva</Button>
        </Form>
    )
