...
/** Build a sub MenuAdmin with items inside and attach collapse behavior */
...
/** Component used to display a header on MenuAdmin when using collapsed/hover mode */
...
class Sidebar extends Component {
  ...
  componentDidMount() {
    ...
    // prepare the flags to handle MenuAdmin collapsed states
  ...
  /** prepare initial state of collapse MenuAdmins. Doesnt allow same route names */
  buildCollapseList = () => {
    let collapse = {};
    MenuAdmin.filter(({ heading }) => !heading).forEach(
      ({ name, path, subMenuAdmin }) => {
        collapse[name] = this.routeActive(
          subMenuAdmin ? subMenuAdmin.map(({ path }) => path) : path
        );
      }
    );
  ...
  getSubRoutes = (item) => item.subMenuAdmin.map(({ path }) => path);

  /** map MenuAdmin config to string to determine which element to render */
  itemType = (item) => {
    if (item.heading) return "heading";
    if (!item.subMenuAdmin) return "MenuAdmin";
    if (item.subMenuAdmin) return "subMenuAdmin";
  };
            ...
            <ul className="sidebar-nav">
              ...
              {MenuAdmin.map((item, i) => {
                ...
                if (this.itemType(item) === "heading")
                  ...
                  if (this.itemType(item) === "MenuAdmin")
                    return (
                      <SidebarItem
                  ...
                  if (this.itemType(item) === "subMenuAdmin")
                    return [
                      <SidebarSubItem
                        item={item}
                      ...
                      >
                        ...
                        {item.subMenuAdmin.map((subitem, i) => (
                          <SidebarItem
                            key={i}
                            item={subitem}
